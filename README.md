...
> **发布状态**：`Brief Yourself 1.0.0` 是首个正式开源版本，采用 MIT License。仓库内的版本包与源码保持一致；GitHub Release 与 tag 不属于本次发布范围。
...
也可以把 `skills/brief-yourself/` 复制到 Agent 的 Skill 目录，或直接使用 `release/brief-yourself-1.0.0.zip`。
...
解压 `release/brief-yourself-1.0.0.zip`。`release/SHA256SUMS.txt` 提供文件大小和 SHA-256，用于确认制品未被替换。
...
`1.0.0` 的合成验收基线为 `95 tests / 0 failures`。发布包包含 `26` 个 canonical Skill 文件，并与 `skills/brief-yourself/` 逐项核对。
